...
# STEP 1: Clear out the docs/ directory
...
# STEP 2: Copy contents of static/ to docs/
    for root, dirs, files in os.walk(static_dir):
        rel_path = os.path.relpath(root, static_dir)
        dest_root = os.path.join(docs_dir, rel_path)
...
def generate_page(from_path, template_path, dest_path, basepath):
    ...
    # Replace fref and src with basepath
    ...
    for content in crawl:
        if os.path.isfile(os.path.join(dir_path_content, content)):
            if content.endswith(".md"):
                ...
                generate_page(os.path.join(dir_path_content,content), template_path, output_html_path, basepath)
